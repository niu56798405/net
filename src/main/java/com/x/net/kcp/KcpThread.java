...
public class KcpThread extends Thread
{
  ...
  public KcpThread(Output out, KcpListerner listerner)
  {
    ...
    kcps = new ConcurrentHashMap<InetSocketAddress, KcpOnUdp>();
...
//          this.kcps.put(dp.sender(), ku);
      ...
      {
        this.kcps.remove((InetSocketAddress) temp.getKcp().getRemote());
      }
  ...
  public KcpOnUdp buildKcp(InetSocketAddress sender,InetSocketAddress local){
	  KcpOnUdp ku = new KcpOnUdp(this.out, sender, local, this.listerner);//初始化     
	  ku.noDelay(nodelay, interval, resend, nc);
      ku.wndSize(sndwnd, rcvwnd);
      ku.setMtu(mtu);
      // conv应该在客户端第一次建立时获取
      ...
      ku.setStream(stream);
      ku.setTimeout(timeout);
      this.kcps.put(sender, ku);
      return ku;
  }
  /**
   * 收到输入
   */
 public void input(DatagramPacket dp)
  {
    if (this.running)
    {
      this.inputs.add(dp);
...
//        ku.release();
  ...
  public KcpOnUdp getKcpOnUdp(InetSocketAddress address){	  	  
	  return kcps.get(address);
  }
}
