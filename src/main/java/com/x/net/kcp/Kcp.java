...
public class Kcp {
	...
	public static final int IKCP_PROBE_INIT = 7000; // 7 secs to probe window
													// size
	public static final int IKCP_PROBE_LIMIT = 120000; // up to 120 secs to
														// probe window
	 ...
	 * @param user
	 */
	public Kcp(Output output, InetSocketAddress remote, InetSocketAddress local) {
	...
	private void flush() {
		...
		for (int i = 0; i < c; i++) {
			if (buffer.readableBytes() + IKCP_OVERHEAD > mtu) {
				this.output.out(buffer, this, remote);
				buffer = PooledByteBufAllocator.DEFAULT.buffer((mtu + IKCP_OVERHEAD) * 3);
			}
